import type { DataRouteMatch } from "../context.js";
import type { History, Location, Path, To } from "./history.js";
import {
...
export const ABSOLUTE_URL_REGEX = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
...
export function createRouter(init: RouterInit): Router {
  ...
  function patchRoutes(routeId: string | null, children: AgnosticRouteObject[]): void {
    let isNonHMR = inFlightDataRoutes == null;
    let routesToUse = inFlightDataRoutes || dataRoutes;
...
function isSameRoute(newRoute: AgnosticRouteObject, existingRoute: AgnosticRouteObject): boolean {
...
const lazyRouteFunctionCache = new WeakMap<AgnosticDataRouteObject, Promise<void>>();
  ...
  return {
    ...
    resolve(handlerOverride) {
      if (
        isUsingNewApi ||
        shouldLoad ||
        (handlerOverride && request.method === "GET" && (match.route.lazy || match.route.loader))
      ) {
