import type { MiddlewareEnabled } from "../types/future.js";
import type { Equal, Expect } from "../types/utils.js";
import type { Location, Path, To } from "./history.js";
 ...
 */
export function unstable_createContext<T>(defaultValue?: T): unstable_RouterContext<T> {
  return { defaultValue };
}
...
function isIndexRoute(route: AgnosticRouteObject): route is AgnosticIndexRouteObject {
 ...
 * @category Utils
 */
export function matchRoutes<RouteObjectType extends AgnosticRouteObject = AgnosticRouteObject>(
...
export function matchRoutesImpl<RouteObjectType extends AgnosticRouteObject = AgnosticRouteObject>(
  routes: RouteObjectType[],
  locationArg: Partial<Location> | string,
  basename: string,
...
interface RouteMeta<RouteObjectType extends AgnosticRouteObject = AgnosticRouteObject> {
  relativePath: string;
  caseSensitive: boolean;
...
interface RouteBranch<RouteObjectType extends AgnosticRouteObject = AgnosticRouteObject> {
  path: string;
  score: number;
 ...
 *   </Route>
 */
export function getPathContributingMatches<T extends AgnosticRouteMatch = AgnosticRouteMatch>(
  matches: T[]
) {
...
export function getResolveToMatches<T extends AgnosticRouteMatch = AgnosticRouteMatch>(
  matches: T[]
) {
